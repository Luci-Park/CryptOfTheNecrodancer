...
namespace cl
{
	...
	{
		if (nullptr == SoundDevice::GetDevice())
	...
	bool AudioClip::LoadWavFile(const std::wstring& path)
	{
		...
		if (FAILED(SoundDevice::GetDevice()->CreateSoundBuffer(&mBufferDesc, &mSoundBuffer, NULL)))
		{
	...
	void AudioClip::Stop(bool reset)
	{
		mSoundBuffer->Stop();
	...
	void AudioClip::SetPosition(float position, bool loop)
	{
		Stop(true);
	...
	int AudioClip::GetDecibel(float volume)
	{
		if (volume > 100.f)
			volume = 100.f;
		else if (volume <= 0.f)
